...
if(isset($_POST["mobile_session"])){
	...
	if(strlen($username) < 5 || strlen($username) > 16 || !preg_match("/^[a-z0-9A-Z ]*$/", $username)){
		$output .= "Username must be between 5 and 16 characters long, and only use characters a-z, 0-9, and A-Z\n";
		$error = true;
	}

	if(strlen($password) < 5){
		$output .= "Password must be at least 5 characters\n";
		$error = true;
	}
	...
	if($error == false){
		...
		if($result && mysqli_num_rows($result) != 0){
			...
			$sql = "INSERT INTO `users` (username, password, inviter_id, status) VALUES ('" . $username . "', '" .  $password ."', '" . $inviter_id. "', 'user')";
...
$output = "<html>
		...
		<title>Create Account geburah.vip</title>
